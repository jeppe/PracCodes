...
""" localtime 返回 tuple 格式的时间
	类似函数 gmtime() 返回 0 时区的值
	localtime 返回当前时
"""
# time.localtime()
...
# output: '2015-08-05 09:23:26'

# time.strftime(ISOTIMEFORMAt, time.localtime(time.time()))

""" 查看时区, 返回值描述当前时区和 0 时区的时间差"""
# time.timezone
...
def ISOString2Time(s):
	...
	"""
	struct_t = time.strptime(s, ISOTIMEFORMAT)

	return time.mktime(struct_t)

def Time2ISOString(s):
	"""
		convert struct_time to format string
	"""

	return time.strftime(ISOTIMEFORMAT, time.lcoaltime( float(s) ))
